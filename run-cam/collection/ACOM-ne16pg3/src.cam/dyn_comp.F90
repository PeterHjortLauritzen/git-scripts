...
subroutine dyn_init(dyn_in, dyn_out)
   ...
   call addfld ('OMEGA_COURANT',   (/ 'lev' /), 'A', '', 'OMEGA courant number', gridname='GLL')!xxx
   call addfld ('RICHARDSON_NUMBER',   (/ 'lev' /), 'A', '','Richardson number', gridname='GLL')!xxx
...
subroutine read_inidat(dyn_in)
   use std_atm_profile,     only: std_atm_pres!xxx phl
   use physconst,           only: gravit!xxx phl
   use physconst,          only: rair!xxx phl
   use air_composition,     only: thermodynamic_active_species_num, dry_air_species_num
   use shr_sys_mod,         only: shr_sys_flush
        ...
        indx = 1
         do j = 1, np
           do i = 1, np
             ! Set qtmp at the unique columns only
             if (pmask(((ie - 1) * npsq) + indx)) then
!xxx               call std_atm_pres(elem(ie)%state%phis(i:i,j)/gravit,elem(ie)%state%psdry(i:i,j))
               phi_s = elem(ie)%state%phis(i,j)
               T_s   = elem(ie)%state%T(i,j,k,1)
               elem(ie)%state%psdry(i:i,j) = phi_s*exp(phi_s/(Rair*T_s))
             else
               elem(ie)%state%psdry(i,j) = 0.0_r8
             end if
             indx = indx + 1
            end do
         end do
