...
!
    ...
    real (kind=r8)                   :: omega_courant(np,np,nlev) !xxx
    ...
    !
!xxx    if (variable_nsplit.or.compute_diagnostics) then    
      if (nsubstep==1) then
        do ie=nets,nete
...
!xxx    if (variable_nsplit.or.compute_diagnostics) then
      do ie=nets,nete
        do k=1,nlev
          do j=1,np
            do i=1,np
              if (dp_end(i,j,k,ie)<dp_start(i,j,k,ie)) then!xxx
                omega_courant(i,j,k) = (dp_end(i,j,k,ie)-dp_start(i,j,k,ie))/dp_start(i,j,k,ie)!xxx
                omega_courant(i,j,k) = (dp_end(i,j,k,ie)-dp_start(i,j,k,ie))/dp_start(i,j,k,ie)!xxx
              else!xxx
                omega_courant(i,j,k) = (dp_end(i,j,k,ie)-dp_start(i,j,k,ie))/dp_start(i,j,k+1,ie)!xxx
                omega_courant(i,j,k) = (dp_end(i,j,k,ie)-dp_start(i,j,k,ie))/dp_start(i,j,k+1,ie)!xxx
              end if!xxx
        ...
        call outfld('OMEGA_COURANT',&
              RESHAPE(omega_courant(:,:,:), (/np*np,nlev/)), np*np, ie)
      end do

!xxx      if (nsubstep==nsplit.and.variable_nsplit) then
!xxx         call t_startf('adjust_nsplit')
!xxx         call adjust_nsplit(elem, tl, hybrid,nets,nete, fvm, omega_cn)
!xxx         call t_stopf('adjust_nsplit')
!xxx      end if
!xxx    end if
